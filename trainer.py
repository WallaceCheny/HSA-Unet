...
from utils import test_single_volume, BoundaryLoss, BoundaryDoULoss
...
def plot_result(dice, h, snapshot_path, args):
    ...
    date_and_time = current_date.strftime("%Y_%m_%d")
...
def trainer_synapse(args, model, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            loss = boundary_loss(outputs, label_batch[:])  # for our boundary DoU loss
            # print("loss-----------", loss)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
def trainer_stomach(args, model, snapshot_path):
    os.makedirs(os.path.join(snapshot_path, 'test'), exist_ok=True)
    test_save_path = os.path.join(snapshot_path, 'test')
    ...
    db_train = Stomach_dataset(base_dir=args.root_path, list_dir=args.list_dir, split="train", img_size=args.img_size,
                               norm_x_transform=x_transforms, norm_y_transform=y_transforms)
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # outputs, aux_out = model(image_batch)
            outputs = model(image_batch)
            # outputs = F.interpolate(outputs, size=label_batch.shape[1:], mode='bilinear', align_corners=False)
            # loss_ce = ce_loss(aux_out, label_batch[:].long())
            # loss_dice = dice_loss(outputs, label_batch, softmax=True)
            # loss = 0.4 * loss_ce + 0.6 * loss_dice
            loss = boundary_loss(outputs, label_batch[:])  # for our boundary DoU loss
            # loss = loss1 + 0.3 * loss_ce
            # print("loss-----------", loss)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        eval_interval = args.eval_interval
        if epoch_num >= int(max_epoch / 2) and (epoch_num + 1) % eval_interval == 0:
            filename = f'{args.model_name}_{args.dataset}_epoch_{epoch_num}.pth'
            save_mode_path = os.path.join(snapshot_path, filename)
            torch.save(model.state_dict(), save_mode_path)
            logging.info("save model to {}".format(save_mode_path))
...
def trainer_isic(args, model, snapshot_path):
    ...
    logging.basicConfig(filename=snapshot_path + f'/{args.model_name}_{args.dataset}_epoch_{args.max_epochs}_log.txt',
                        level=logging.INFO,
                        format='[%(asctime)s.%(msecs)03d] %(message)s', datefmt='%H:%M:%S')
    logging.getLogger().addHandler(logging.StreamHandler(sys.stdout))
    logging.info(str(args))
    ...
    db_train = Isic_dataset(base_dir=args.root_path, list_dir=args.list_dir, split="train", img_size=args.img_size,
                            norm_x_transform=x_transforms, norm_y_transform=y_transforms)
    ...
    db_test = Isic_dataset(base_dir=args.test_path, split="test_vol", list_dir=args.list_dir, img_size=args.img_size)
    testloader = DataLoader(db_test, batch_size=1, shuffle=False, num_workers=1)
    ...
    boundary_loss = BoundaryDoULoss(num_classes)
    optimizer = optim.SGD(model.parameters(), lr=base_lr, momentum=0.9, weight_decay=0.0001)
    writer = SummaryWriter(snapshot_path + f'/{args.model_name}_{args.dataset}_epoch_{args.max_epochs}_log')
    iter_num = 0
    max_epoch = args.max_epochs
    max_iterations = args.max_epochs * len(trainloader)  # max_epoch = max_iterations // len(trainloader) + 1
    logging.info("{} iterations per epoch. {} max iterations ".format(len(trainloader), max_iterations))
...
def trainer_cvc(args, model, snapshot_path):
    ...
    db_train = CVC_dataset(base_dir=args.root_path, list_dir=args.list_dir, split="train", img_size=args.img_size,
                           norm_x_transform=x_transforms, norm_y_transform=y_transforms)
    ...
    db_test = CVC_dataset(base_dir=args.test_path, split="test_vol", list_dir=args.list_dir, img_size=args.img_size)
    testloader = DataLoader(db_test, batch_size=1, shuffle=False, num_workers=1)
    ...
    for epoch_num in iterator:
        ...
        if epoch_num >= max_epoch - 1:
            ...
            if not (epoch_num + 1) % args.eval_interval == 0:
                logging.info("*" * 20)
                logging.info(f"Running Inference after epoch {epoch_num} (Last Epoch)")
                print(f"Epoch {epoch_num}, Last Epoch")
                mean_dice, mean_hd95 = inference(model, testloader, args, test_save_path=test_save_path)
                dice_.append(mean_dice)
                hd95_.append(mean_hd95)
                model.train()
