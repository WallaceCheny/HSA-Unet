...
class Stomach_dataset(Dataset):
    def __init__(self, base_dir, list_dir, split, img_size, norm_x_transform=None, norm_y_transform=None):
        ...
        self.split = split
        with open(os.path.join(list_dir, self.split + '.txt')) as file:
            self.sample_list = [line.strip() for line in file.readlines()]
        self.data_dir = base_dir
        self.img_path = os.path.join(base_dir, "images")
        self.label_path = os.path.join(base_dir, "masks")
        # self.sample_list = os.listdir(self.img_path)
        ...
        #             albu.Resize(self.output_size, self.output_size, always_apply=True, interpolation=cv2.INTER_LANCZOS4),
        #             albu.HorizontalFlip(),
        #             albu.VerticalFlip(),
        #             albu.ColorJitter(brightness=(0.6, 1.6), contrast=0.2, saturation=0.1, hue=0.01),
        #             albu.Affine(scale=(0.5, 1.5), translate_percent=(-0.15, 0.15), rotate=(-12, 12), shear=(-22.5, 22)),
        #             albu.Normalize()
        #         }
    ...
    def __getitem__(self, idx):
        ...
        label_path = os.path.join(self.label_path, self.sample_list[idx])

        image, label = Image.open(img_path).convert('RGB'), Image.open(label_path).convert('L')
        ...
        sample['case_name'] = "case" + self.sample_list[idx].strip('\n') #str(idx)
        return sample
