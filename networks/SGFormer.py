...
class Attention(nn.Module):
    def __init__(self, dim, mask, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1,
        ...
        torch.nn.init.trunc_normal_(self.relative_position_bias_table, mean=0, std=.02)
        # trunc_normal_(self.relative_position_bias_table, mean=0, std=.02)
    ...
    def forward(self, x, H, W, mask):
        ...
        if self.sr_ratio > 1:
            if mask is None:
                ...
                relative_position_bias = relative_position_bias[0].unsqueeze(0)  # B, nH, Wh*Ww, Wh*Ww
                # B, nH, wh1, wh2 = relative_position_bias.shape
                # relative_position_bias = relative_position_bias.view(-1,wh1, wh2)
                attn2 = attn2 + relative_position_bias
