...
class EfficientAdditiveAttnetion(nn.Module):
    ...
    def forward(self, x):
        # Step 1: 嵌入矩阵x用两个矩阵to_query, to_key, Wk变换为查询(Q)
        # 和键(K)
        ...
        # Step 2: 将查询矩阵Q乘以可学习参数向量w_g，学习查询的注意权值，得到全局关注查询向量A
        query_weight = query @ self.w_g  # BxNx1 (BxNxD @ Dx1)
        A = query_weight * self.scale_factor  # BxNx1

        A = torch.nn.functional.normalize(A, dim=1)  # BxNx1

        # Step 3: 然后，根据学习到的注意权值对查询矩阵进行池化，得到单个全局查询向量，如下所示
...
class DualTransformerBlock(nn.Module):
    ...
    def __init__(self, in_dim, key_dim, value_dim, head_count=1, token_mlp="mix"):
        ...
        self.attn = EfficientAdditiveAttnetion(in_dims=in_dim, token_dim=in_dim, num_heads=1)
